...
namespace SizeUp.Core.DataLayer.Base
{
    public class ZipCode
    {
        public static IQueryable<Data.ZipCode> In(SizeUpContext context, long placeId, Granularity boundingGranularity)
        {
            ...
            {
                output = context.ZipCodes.Where(i => i.ZipCodePlaceMappings.Any(z => z.CityCountyMapping.City.CityCountyMappings.Any(m => m.Id == placeId)));
            }
            else if (boundingGranularity == Granularity.County)
            {
                output = context.ZipCodes.Where(i => i.ZipCodePlaceMappings.Any(z => z.CityCountyMapping.County.CityCountyMappings.Any(m => m.Id == placeId)));
            }
            else if (boundingGranularity == Granularity.Metro)
            {
                output = context.ZipCodes.Where(i => i.ZipCodePlaceMappings.Any(z => z.CityCountyMapping.County.Metro.Counties.Any(co => co.CityCountyMappings.Any(m => m.Id == placeId)))); 
            }
            else if (boundingGranularity == Granularity.State)
            {
                output = context.ZipCodes.Where(i => i.ZipCodePlaceMappings.Any(z => z.CityCountyMapping.County.State.Counties.Any(co => co.CityCountyMappings.Any(m => m.Id == placeId))));
            }
