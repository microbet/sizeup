        ...
        {     
            foreach (var geo in Geographies)
            {
                using (GraphicsPath gp = new GraphicsPath())
                {
                    ...
                    Color c = Color.FromArgb(Opacity, ColorTranslator.FromHtml(geo.Color));
                    SolidBrush brush = new SolidBrush(c);                  
                    Graphics.FillPath(brush, gp);
                    if (BorderWidth > 0)
                    {
                        Pen p = new Pen(Color.FromArgb(BorderOpacity, ColorTranslator.FromHtml(BorderColor)),BorderWidth);
