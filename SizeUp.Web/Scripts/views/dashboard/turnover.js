(function () {
    ...
    sizeup.views.dashboard.turnover = function (opts) {
        ...
        var turnoverPercentileDataReturned = function (data) {
            ...
            if (data.Metro) {
                me.data.turnover.percentiles.Metro = data.Metro.Percentile < 1 ? 'more than 99%' : data.Metro.Percentile > 99 ? 'less than 99%' : 'less than or equal to ' + data.Metro.Percentile + '%';
            }
            if (data.State) {
        ...
        var jobChangePercentileDataReturned = function (data) {
            ...
            if (data.Metro) {
                me.data.jobChange.percentiles.Metro = data.Metro.Percentile < 1 ? 'less than 99%' : data.Metro.Percentile > 99 ? 'greater than 99%' : 'greater than or equal to ' + data.Metro.Percentile + '%';
            }
            if (data.State) {
                me.data.jobChange.percentiles.State = data.State.Percentile < 1 ? 'less than 99%' : data.State.Percentile > 99 ? 'greater than 99%' : 'greater than or equal to ' + data.State.Percentile + '%';
            }
            if (data.Nation) {
                me.data.jobChange.percentiles.Nation = data.Nation.Percentile < 1 ? 'less than 99%' : data.Nation.Percentile > 99 ? 'greater than 99%' : 'greater than or equal to ' + data.Nation.Percentile + '%';
            }
